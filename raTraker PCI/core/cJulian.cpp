...
cJulian::cJulian(time_t time)
{
   ...
   //gmtime_s(&stm, &time); /*OLD*/
   memcpy(&stm, gmtime(&time), sizeof(struct tm)); /*NEW (RCCR)*/
...
//      // long fromLocal;           /*OLD*/
//      //_get_timezone(&fromLocal); /*OLD*/
//      //tEpoch -= fromLocal;       /*OLD*/

//      tEpoch -= timezone; /*Note: Be sure call before function _tzset();*/
